"""Deep report specific postprocess hook."""
...
_PROMPT_PATH = Path(__file__).parent / "prompt" / "postprocess_deep_report.md"
_SEP = "\n\n---\n\n"
_USER_SECTION_KEYS = ("skin", "lifestyle", "goals")
...
def _load_prompt() -> str:
    ...
    return "你是深度报告后台档案维护员。根据本轮对话更新 USER.md 或 SOUL.md，完成后回 done."
    ...
    *,
    ...
    if first_token_reply:
        ...
        turn_text = (
            "## 本轮深度报告对话\n\n"
            f"用户：{user_message}\n\n"
            f"深度报告开场（first_token_llm）：{first_token_reply}\n\n"
            f"深度报告正式回复：{formal_reply}"
        )
    else:
        turn_text = f"## 本轮深度报告对话\n\n用户：{user_message}\n\n深度报告：{assistant_reply}"

    return _SEP.join([
        turn_text,
        f"## 当前 USER.md\n\n{user_md or '（暂无，首次写入）'}",
        f"## 当前 SOUL.md（沟通偏好 / 红线）\n\n{soul_md or '（暂无，首次写入）'}",
    ])
...
class _UpdateDeepReportUserSectionTool(_TrackedDocumentTool):
    name = "update_deep_report_user_section"
    description = (
        "Patch one USER.md section used by deep report personalization. "
        "The section must be one of skin, lifestyle, goals. Pass only the section body; "
        "third-level headings are allowed, but do not include #/## headings."
    )
    parameters = {
        "type": "object",
        "properties": {
            "section": {
                ...
                "description": "USER.md section to patch.",
            },
            "content": {
                "type": "string",
                "description": "Full new body for this section only. Do not include #/## headings.",
            },
        },
    ...
    async def execute(self, *, section: str, content: str) -> ToolResult:
        ...
        if section == "skin" and _contains_managed_profile_content(body):
        ...
        updated = update_user_section(existing or "", section, body)
...
class DeepReportPostprocessHook(PostrunHook):
    """Post-turn document maintenance for the deep report sub-agent."""
    ...
    async def on_turn_end(self, ctx: TurnContext) -> TaskExecutionResult:
        try:
            ...
            logger.warning(
                "deep report postprocess failed tenant={} session={}：{}",
                ctx.tenant_key, ctx.session_key, exc,
            )
            return TaskExecutionResult.failed(
                f"deep report postprocess failed: {exc}",
                summary="deep report postprocess execution failed",
            )
    ...
    async def _run(self, ctx: TurnContext) -> TaskExecutionResult:
        ...
        for tool in (
            _UpdateDeepReportUserSectionTool(ctx.tenant_key, self._doc_repo, changed_docs),
        ...
        if error_message:
            return TaskExecutionResult.failed(
                error_message,
                summary="deep report postprocess react loop returned error",
            )

        logger.debug(
            "deep report postprocess completed tenant={} session={}",
        ...
        return TaskExecutionResult.noop(
            summary="deep report postprocess completed with no document changes",
