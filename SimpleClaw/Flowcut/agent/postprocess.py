...
每轮用户对话结束后，该 Hook 将：
  1. 从数据库获取该租户当前的 USER.md / SOUL.md / BOUNDARIES.md
...
from typing import Any  # TODO: 实现后替换
...
# TODO: 实现后替换 — Flowcut.storage 暂时没有 document_repo
# from Flowcut.storage.document_repo import DocumentRepository
...
class _UpdateUserSectionTool(_TrackedDocumentTool):
    ...
    description = (
        ...
        "without top-level markdown section headings. System-managed sections such as "
        "## Learned Skin Profile are never editable by this tool."
    )
    parameters = {
        "type": "object",
        "properties": {
            ...
            "content": {
                "type": "string",
                "description": "New body for this section only. Do not include #/## section headings or system-managed profile fields.",
            },
        },
...
class _AppendSoulNoteTool(_TrackedDocumentTool):
    ...
    description = "Append one stable communication preference to SOUL.md. Pass a single note, without '- ' prefix."
    parameters = {
        "type": "object",
        "properties": {
            "note": {
                "type": "string",
                "description": "One stable communication preference explicitly stated by the user.",
            },
        },
        "required": ["note"],
    }
...
class _RemoveSoulNoteTool(_TrackedDocumentTool):
    ...
    description = "Remove one obsolete communication preference from SOUL.md."
    parameters = {
        "type": "object",
        "properties": {
            "note": {
...
class _AppendBoundaryTool(_TrackedDocumentTool):
    name = "append_boundary"
    description = "Append one user red-line / refusal item to BOUNDARIES.md. Pass a single item, without '- ' prefix."
    parameters = {
        "type": "object",
        "properties": {
            "item": {
                "type": "string",
                "description": "One hard refusal or red-line explicitly stated by the user.",
            },
        },
        "required": ["item"],
    }
    ...
    async def execute(self, *, item: str) -> ToolResult:
        boundary = _normalize_markdown_item(item)
        if not boundary:
            return ToolResult(content="Error: item must not be empty.", ok=False)
        existing = await self._doc_repo.get(self._tenant_key, "BOUNDARIES.md")
        updated = append_markdown_item(existing or "", boundary)
        if (existing or "") == updated:
            return ToolResult(content="noop: BOUNDARIES.md unchanged")
        await self._doc_repo.set(self._tenant_key, "BOUNDARIES.md", updated)
        self._mark_changed("BOUNDARIES.md")
        return ToolResult(content="ok: BOUNDARIES.md boundary appended")


class _RemoveBoundaryTool(_TrackedDocumentTool):
    name = "remove_boundary"
    description = "Remove one obsolete red-line / refusal item from BOUNDARIES.md."
    parameters = {
        "type": "object",
        "properties": {
            "item": {
                "type": "string",
                "description": "Existing red-line item to remove when the user explicitly re-opens that topic.",
            },
        },
        "required": ["item"],
    }
    ...
    async def execute(self, *, item: str) -> ToolResult:
        ...
        if not boundary:
            return ToolResult(content="Error: item must not be empty.", ok=False)
        existing = await self._doc_repo.get(self._tenant_key, "BOUNDARIES.md")
        updated = remove_markdown_item(existing or "", boundary)
        if (existing or "") == updated:
            return ToolResult(content="noop: BOUNDARIES.md unchanged")
        await self._doc_repo.set(self._tenant_key, "BOUNDARIES.md", updated)
        self._mark_changed("BOUNDARIES.md")
        return ToolResult(content="ok: BOUNDARIES.md boundary removed")
_SEP = "\n\n---\n\n"
...
def _render_input(
    *,
    ...
    if soul_md:
        parts.append(f"## 当前 SOUL.md（此用户沟通偏好）\n\n{soul_md}")
    else:
        parts.append("## 当前 SOUL.md（此用户沟通偏好）\n\n（暂无，首次写入）")

    if boundaries_md:
        parts.append(f"## 当前 BOUNDARIES.md（此用户红线）\n\n{boundaries_md}")
    else:
        parts.append("## 当前 BOUNDARIES.md（此用户红线）\n\n（暂无，首次写入）")
...
class PostprocessHook(PostrunHook):
    ...
    async def _run(self, ctx: TurnContext) -> TaskExecutionResult:
        ...
        input_text = _render_input(
            ...
            user_md=user_md or "",
            soul_md=soul_md or "",
            boundaries_md=boundaries_md or "",
        ...
        for tool in (
            ...
            _AppendBoundaryTool(ctx.tenant_key, self._doc_repo, changed_docs),
            _RemoveBoundaryTool(ctx.tenant_key, self._doc_repo, changed_docs),
        ):
