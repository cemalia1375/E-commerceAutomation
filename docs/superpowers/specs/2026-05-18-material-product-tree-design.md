...
后端 `GET /materials/tree-summary` 直接返回 Ant Design Tree 节点格式，前端类型与之对齐：

```typescript
// 直接对应后端 tree-summary 响应，供 Ant Design Tree 消费
interface TreeNode {
  key: string        // 产品节点: "雪莲洗液"；场景角色节点: "雪莲洗液|医生"
  title: string      // "雪莲洗液 (80)" | "医生 (45)"
  children?: TreeNode[]
}

// 解析 key 取出结构化字段的工具函数（store 内部使用）
// 产品节点: key.split('|') = [product]
// 场景角色节点: key.split('|') = [product, sceneRole]
...
| GET | `/materials/tree-summary` | ✅ 已实现 | 返回 Ant Design Tree 格式（见下方），前端直接用此接口，**不需要新建 `/materials/tree`** |
...
### GET /materials/tree-summary 现有响应格式（Ant Design Tree 直用）
...
[
  {
    ...
    "title": "雪莲洗液 (80)",
    "children": [
      { "key": "雪莲洗液|医生", "title": "医生 (45)" },
      { "key": "雪莲洗液|药材", "title": "药材 (12)" }
    ]
  },
  { "key": "通用", "title": "通用 (15)", "children": [] }
]
```

前端 `productTreeStore.fetchTree()` 直接消费此格式，从 `key` 字段解析 `product` 和 `sceneRole`（按 `|` 分割）。`ProductNode` / `SceneRoleNode` 类型定义相应调整为 `key: string`。
...
| 新增 | `src/api/products.ts` | `getProductTree(tenantKey)` → `TreeNode[]`（调 `/materials/tree-summary`）；`getProducts(tenantKey)` → `string[]`（调 `/materials/products`） |
