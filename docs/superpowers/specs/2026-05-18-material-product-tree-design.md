...
interface ProductTreeState {
  ...
  addNode: (product: string, sceneRole?: string) => void  // zip 导入后刷新
}
```
...
| 方法 | 路径 | 变更 |
|------|------|------|
| GET | `/materials` | 新增可选查询参数 `product` / `scene_role` |
| POST | `/materials/upload` | FormData 新增 `product`（必填）、`scene_role`（选填） |
| GET | `/materials/tree` | **新增**：返回产品树（含数量） |
| GET | `/products` | 已存在于设计文档，确认可用 |
| POST | `/materials/upload-zip` | **新增**：解析 zip，返回预览 |
| POST | `/materials/upload-zip/confirm` | **新增**：确认导入，触发写入 |

### GET /materials/tree 响应
...
| 新增 | `src/api/products.ts` | `getProductTree(tenantKey)` → `ProductNode[]`；`getProducts(tenantKey)` → `string[]` |
