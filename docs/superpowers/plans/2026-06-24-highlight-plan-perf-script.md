...
**Goal:** 创建 `Flowcut/scripts/perf_highlight_plan.py`，复现 `make_highlight_plan_executor` 的完整流程并在三个 Stage 边界打计时，跑完输出 `perf_highlight_<drama>_<ts>.json`。

**Architecture:** 独立脚本，直接 import 真实服务函数（`analyze_video`、`detect_scene_cuts`、`select_start_shots`、ffmpeg helpers），自建 DB pool 和 OSS client，不走 TaskQueue；向 DB 写入真实 creative 记录，但 compose 提交只记录耗时不实际入队（避免触发 worker 流程）。

**Tech Stack:** Python asyncio, aiomysql, `Flowcut.*` 现有模块, `time.perf_counter`

## Global Constraints

- 工作目录：`SimpleClaw/`，所有命令从这里跑
- 运行方式：`uv run python -m Flowcut.scripts.perf_highlight_plan <drama_name> [--candidates N] [--tenant-key KEY]`
- 不修改任何 production 文件
- 需要 `.env` 中有效的 `GOOGLE_API_KEY`、`MYSQL_*`、`FLOWCUT_OSS_*` 配置
...
| 路径 | 操作 | 职责 |
|------|------|------|
| `Flowcut/scripts/perf_highlight_plan.py` | 新建 | 全部逻辑：argparse、DB 初始化、Stage A/B/C 计时、JSON 输出 |

---

### Task 1：脚本骨架 — argparse + DB/OSS 初始化 + JSON 输出框架

**Files:**
- Create: `SimpleClaw/Flowcut/scripts/perf_highlight_plan.py`

**Interfaces:**
- Produces: `async def main()` 入口，`PerfResult` TypedDict，`write_result(result, drama_name)` 函数
...
- [ ] **Step 2: 验证脚本可以 import，--help 不报错**

```bash
cd SimpleClaw
uv run python -m Flowcut.scripts.perf_highlight_plan --help
```

期望输出：显示 usage 说明，无 ImportError。

- [ ] **Step 3: 提交**

```bash
git add Flowcut/scripts/perf_highlight_plan.py
git commit -m "feat(perf): 高光规划性能测试脚本骨架"
```

---

### Task 2：Stage A — 下载 + 归一化 + 合并 + Gemini 整体拆镜

**Files:**
- Modify: `SimpleClaw/Flowcut/scripts/perf_highlight_plan.py`

**Interfaces:**
- Consumes: `run_perf()` 占位函数，`highlight_asset_repo`、`oss_client`
- Produces: `StageA` 结果字典，`norm_cache: dict[int, tuple[str, float]]`（已归一化路径 + 时长，供 Stage C 复用）

- [ ] **Step 1: 在 `run_perf` 里实现 Stage A**

将 `run_perf` 的 `raise NotImplementedError` 替换为以下完整实现（Stage B/C 部分后续 Task 补充，现在先 `raise NotImplementedError` 占位）：
...
from Flowcut.services.clip_planner import (
    ...
    WINDOW, IDEAL, MAX_FORWARD_EPISODES, _HIGHLIGHT_SPAN_PAD_S if hasattr(...) else 8.0,
...
> **注意**：`_HIGHLIGHT_SPAN_PAD_S` 是 `executors.py` 里的模块级常量（值 `8.0`），直接 import。

完整的 `run_perf` 函数如下（替换原来的 `raise NotImplementedError`）：
...
async def run_perf(
    *,
    ...
    async def ensure_episode(asset: dict) -> tuple[str, float]:
        aid = int(asset["id"])
        if aid in norm_cache:
            return norm_cache[aid]
        ...
        return norm_cache[aid], dl_s, norm_s  # type: ignore[return-value]
    ...
    try:
        ...
        for asset in head:
            ...
            (norm, dur), dl_s, norm_s = await ensure_episode(asset)  # type: ignore
        ...
        # Stage B/C 由后续 Task 填充
        raise NotImplementedError("Stage B/C 待实现")

    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
```

- [ ] **Step 2: 用真实剧名跑 Stage A，确认打印出耗时且不报错（Stage B NotImplementedError 是预期行为）**

```bash
cd SimpleClaw
uv run python -m Flowcut.scripts.perf_highlight_plan "你的剧名" --candidates 2
```

期望输出：
```
ep1: download=8.3s normalize=12.1s
ep2: download=7.1s normalize=11.4s
ep3: download=6.9s normalize=11.8s
merge=0.4s
gemini=28.6s detect_cuts=2.1s shots=42
Traceback ... NotImplementedError: Stage B/C 待实现
```
...
```bash
git add Flowcut/scripts/perf_highlight_plan.py
git commit -m "feat(perf): Stage A 计时 — 下载/归一化/合并/Gemini"
```

---

### Task 3：Stage B — select_start_shots 计时

**Files:**
- Modify: `SimpleClaw/Flowcut/scripts/perf_highlight_plan.py`

**Interfaces:**
- Consumes: `head_shots`（来自 Task 2 Stage A）, `offsets`, `durations`
- Produces: `StageB` 结果字典，`candidates: list[StartCandidate]`

- [ ] **Step 1: 将 Stage B 的 `raise NotImplementedError` 替换为实现**

把 `raise NotImplementedError("Stage B/C 待实现")` 那一行替换成：
...
```python
        ...
        # Stage C 由后续 Task 填充
        raise NotImplementedError("Stage C 待实现")
```

- [ ] **Step 2: 跑验证，确认 Stage B 打印出耗时（Stage C NotImplementedError 是预期行为）**
...
```
...（Stage A 输出）...
Stage B: select=5.1s candidates=2
Traceback ... NotImplementedError: Stage C 待实现
```
...
```bash
git add Flowcut/scripts/perf_highlight_plan.py
git commit -m "feat(perf): Stage B 计时 — select_start_shots"
```

---

### Task 4：Stage C — 每个候选的 span 细拆 + DB 写入 + JSON 输出

**Files:**
- Modify: `SimpleClaw/Flowcut/scripts/perf_highlight_plan.py`

**Interfaces:**
- Consumes: `candidates`（来自 Task 3），`ep_index`、`norm_cache`、`tmp_dir`
- Produces: `StageC` 结果字典，完整 `PerfResult`，JSON 文件

- [ ] **Step 1: 将 Stage C 的 `raise NotImplementedError` 替换为实现**

把 `raise NotImplementedError("Stage C 待实现")` 替换成（仍在 `try` 块内，`finally` 块之前）：

```python
        # ── Stage C ────────────────────────────────────────────────────────
        stage_c_t0 = time.perf_counter()
        target_len = WINDOW[1] + _HIGHLIGHT_SPAN_PAD_S
        candidate_timings: list[CandidateTiming] = []
        ...
        for cand in candidates:
            ...
            for i, (src, cs, ce) in enumerate(seg_specs):
                out = os.path.join(tmp_dir, f"span_{cand.episode_no}_{i}.mp4")
            ...
            if len(span_cut_paths) == 1:
                ...
                span_path = os.path.join(tmp_dir, f"span_{cand.episode_no}.mp4")
                sl = os.path.join(tmp_dir, f"span_{cand.episode_no}_concat.txt")
...
- [ ] **Step 2: 端到端跑一次，确认整体走通且 JSON 文件生成**
...
期望输出：
```
ep1: download=...s normalize=...s
...
Stage B: select=...s candidates=2
  候选 ep1: dl_new=...s cut=...s gemini=...s pick=...s db=...s creative_id=123
  候选 ep3: ...
=== 测试完成 ===
总耗时：118.4s
Stage A：75.2s
Stage B：5.1s
Stage C：38.1s
结果已写入：perf_highlight_你的剧名_1719244800.json
```

确认 JSON 文件存在且字段完整：

```bash
cat perf_highlight_*.json | python3 -c "import sys, json; d=json.load(sys.stdin); print(list(d.keys()))"
```

期望输出：`['drama_name', 'tenant_key', 'num_candidates_requested', 'total_elapsed_s', 'stage_A', 'stage_B', 'stage_C']`

- [ ] **Step 3: 提交**

```bash
git add Flowcut/scripts/perf_highlight_plan.py
git commit -m "feat(perf): Stage C + JSON 输出 — 高光规划完整性能测试"
```
...
**Spec coverage 检查：**
- ✅ 独立脚本，不修改 production 代码
- ✅ 三段粗粒度计时：Stage A（每集 download/normalize + merge + gemini + scene_cuts）、Stage B（select_start_shots）、Stage C（每候选 cut/gemini/pick/db）
- ✅ 写入真实 DB（creative_repo.create_cross_episode_job）
- ✅ compose 提交 stub（不实际入队，避免触发 worker）
- ✅ 输出 JSON 文件
- ✅ argparse：drama_name、--candidates、--tenant-key

**Placeholder 扫描：** 无 TBD/TODO。

**类型一致性：**
- `EpisodeTiming`、`StageA`、`StageB`、`CandidateTiming`、`StageC`、`PerfResult` 均在 Task 1 定义，Task 2-4 直接使用。
- `ensure_episode` 返回 `(norm_path, dur), dl_s, norm_s`，Task 2/4 两处调用保持一致（都用 `(norm, dur), dl_s, norm_s = await ensure_episode(asset)`）。

**已知边界：** `creative_repo.create_cross_episode_job` 的签名需与 `CreativeRepository` 实际方法名一致（executor 中有用到，可在 Task 4 Step 1 执行前 `grep -n "create_cross_episode_job" Flowcut/storage/creative_repo.py` 确认）。
